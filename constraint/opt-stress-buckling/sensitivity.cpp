                   ...
                   std::vector<BucklingLinearElastic> &element)
{
  ...
  for(int i = 0; i < fem.nelx; i++)
  {
    ...
    misesPsum += pow(misesQP / fem.limstress, fem.pNorm);
  }
...
vector<double> sens_stress(FEMBuckling &fem,
                           ...
                           vector<Node> &node, const SparseMatrix<double> &K)
{
  ...
  for(int i = 0; i < fem.nelx; i++)
  {
    misesQP[i] = pow(element[i].design_s(), fem.qp) * element[i].young2() *
                 element[i].mises() / element[i].young();
    misesPsum += pow(misesQP[i] / fem.limstress, fem.pNorm);
  }
  ...
  for(int nel = 0; nel < fem.nelx; nel++)
  {
    ...
    if(fem.ndim == 2)
    {
      ...
      for(int i = 0; i < fem.voigt; i++)
        facEle1[i] = dpndvm * pow(misesQP[nel] / fem.limstress, fem.pNorm - 1) *
                     dvmdsig[i] / fem.limstress;
  ...
  for(int nel = 0; nel < fem.nelx; nel++)
  {
    ...
    dgds[nel] = factor12[nel] - factor34[nel];
  } /*element loop end*/
